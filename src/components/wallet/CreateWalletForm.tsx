...
export default function CreateWalletForm() {
  ...
  const { createInAppWallet, isConnecting } = useWalletStore()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createInAppWallet(password)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="bg-blue-900 border border-blue-700 rounded-lg p-3 text-blue-100 text-sm">
        <p className="font-medium mb-1">Security Tips:</p>
        <ul className="list-disc list-inside space-y-1 text-xs">
          <li>Use a strong, unique password</li>
          <li>Store your seed phrase securely</li>
